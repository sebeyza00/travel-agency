...
- [x] **AUDIT-DATA-003**: On server startup, the system shall idempotently add the nullable `agent_email` column to the `bookings` table when it is absent.
...
- [x] **AUDIT-API-005**: When writing an `audit_log` row, the system shall set `actor` to the booking agent's email when an agent is provided, and to "internal-agent" when none is provided.
- [x] **AUDIT-API-008**: When persisting a booking, the system shall record the booking agent's email on the `bookings.agent_email` column when an agent is provided, and null when none is provided.
